...
import { 
  FiCreditCard, FiLock, FiArrowLeft, FiCheck, FiX, 
  FiShield, FiCalendar, FiUser, FiMail 
} from "react-icons/fi";
import { motion } from "framer-motion";
import { auth, userSubscriptions, subscriptionPlans, users } from "../lib/supabase";
...
const PaymentPage = () => {
  ...
  useEffect(() => {
    const initializePayment = async () => {
      ...
      try {
        ...
        // Get user profile
        const { data: profile, error: profileError } = await users.getById(currentUser.id);
        if (profile) {
          setUserProfile(profile);
          setPaymentForm(prev => ({
            ...prev,
            cardholderName: `${profile.first_name || ''} ${profile.last_name || ''}`.trim() || currentUser.email
          }));
        }
        
        // Get plan details
        if (planId) {
          const { data: plans, error: planError } = await subscriptionPlans.getAll();
  ...
  const handlePayment = async (e) => {
    ...
    try {
      ...
      const { data: subscription, error: subscriptionError } = await userSubscriptions.create(subscriptionPayload);
      ...
      const qrData = {
        ...
        planType: billingType,
        gymAccess: selectedPlan.gym_access_description,
        price: billingType === 'monthly' ? selectedPlan.price_monthly : selectedPlan.price_yearly,
        currency: "USD"
      };
                ...
                >
                  {isProcessing ? (
                    ...
                    <>
                      ...
                      Complete Payment - ${billingType === 'monthly' ? selectedPlan?.price_monthly : selectedPlan?.price_yearly}
                    </>
                  )}
                </button>
              </form>
            ...
            >
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <span className="text-xl font-bold">
                      ${billingType === 'monthly' ? selectedPlan?.price_monthly : selectedPlan?.price_yearly}
                    </span>
                    <p className="text-sm text-light-textSecondary dark:text-dark-textSecondary">
              ...
              <div className="mt-6 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                ...
                <p className="text-sm text-green-700 dark:text-green-400">
                  Get instant access with your QR code membership card after payment.
                </p>
              </div>
            </motion.div>
          </div>
